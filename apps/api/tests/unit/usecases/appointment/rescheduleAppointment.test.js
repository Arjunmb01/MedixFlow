...
const rescheduleAppointment_usecase_1 = require("@/application/use-cases/appointment/rescheduleAppointment.usecase");
...
describe('RescheduleAppointmentUseCase', () => {
    ...
    beforeEach(() => {
        ...
        useCase = new rescheduleAppointment_usecase_1.RescheduleAppointmentUseCase(mockAppointmentRepo, mockSchedulingPolicy, mockDateTimeService, mockSendNotificationUseCase);
        jest.clearAllMocks();
    });
    it('should successfully reschedule when initiated by patient', async () => {
        ...
        expect(result.slotStart).toBe('11:00');
    });
    it('should create a proposal when initiated by doctor', async () => {
        ...
        expect(mockSendNotificationUseCase.execute).toHaveBeenCalledWith(expect.objectContaining({
            ...
            type: notification_types_1.NotificationType.RESCHEDULE_PROPOSAL
        }));
