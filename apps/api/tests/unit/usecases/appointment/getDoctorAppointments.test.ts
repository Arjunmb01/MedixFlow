import { GetDoctorAppointmentsUseCase } from "@/application/use-cases/appointment/getDoctorAppointments.usecase";

describe('GetDoctorAppointmentsUseCase', () => {
    let useCase: GetDoctorAppointmentsUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new GetDoctorAppointmentsUseCase(mockAppointmentRepo, mockDateTimeService);
        jest.clearAllMocks();
    });
    ...
    it('should filter out upcoming appointments if isUpcoming filter is undefined', async () => {
        ...
        mockDateTimeService.isUpcoming.mockImplementation((date: any, slot: string) => slot === '14:00');
