...
const ApplyLeaveUseCase_1 = require("@/application/use-cases/leave/ApplyLeaveUseCase");
describe('ApplyLeaveUseCase', () => {
    let useCase;
    let mockLeaveRepo;
    let mockAppointmentRepo;
    let mockSendNotificationUseCase;
    let mockCancelAppointmentUseCase;
    const mockInput = {
    ...
    beforeEach(() => {
        ...
        useCase = new ApplyLeaveUseCase_1.ApplyLeaveUseCase(mockLeaveRepo, mockAppointmentRepo, mockSendNotificationUseCase, mockCancelAppointmentUseCase);
        jest.clearAllMocks();
    });
    ...
    it('should auto-cancel appointments if conflicts exist and suppressed', async () => {
        ...
        expect(mockCancelAppointmentUseCase.execute).toHaveBeenCalledWith('a-1', 'p-1', 'Doctor on Leave', false, true);
        expect(mockLeaveRepo.create).toHaveBeenCalled();
    });
});
