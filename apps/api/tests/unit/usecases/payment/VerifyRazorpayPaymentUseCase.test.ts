...
describe('VerifyRazorpayPaymentUseCase', () => {
    let useCase: VerifyRazorpayPaymentUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new VerifyRazorpayPaymentUseCase(mockPaymentRepo, mockRazorpayService, mockConfirmPaymentUseCase);
        jest.clearAllMocks();
    });
    ...
    it("should return success if payment is already PAID", async () => {
        // Arrange
        mockRazorpayService.verifyPaymentSignature.mockReturnValue(true);
        mockPaymentRepo.findByOrderId.mockResolvedValue({ status: PaymentStatus.PAID });
