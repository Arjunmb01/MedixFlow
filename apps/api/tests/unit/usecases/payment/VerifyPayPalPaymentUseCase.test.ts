...
describe('VerifyPayPalPaymentUseCase', () => {
    let useCase: VerifyPayPalPaymentUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new VerifyPayPalPaymentUseCase(mockPaymentRepo, mockConfirmPaymentUseCase);
        jest.clearAllMocks();
    });
    ...
    it('should return success if already PAID', async () => {
        mockPaymentRepo.findByPayPalOrderId.mockResolvedValue({ id: 'p-1', status: PaymentStatus.PAID });
        const result = await useCase.execute('order-1');
        expect(result.success).toBe(true);
