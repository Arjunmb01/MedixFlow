...
describe("HandlePayPalWebhookUseCase", () => {
  let useCase: HandlePayPalWebhookUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new HandlePayPalWebhookUseCase(mockPaymentRepo, mockConfirmPaymentUseCase);
  });
  ...
  it("should confirm payment when CHECKOUT.ORDER.APPROVED event is received", async () => {
    ...
    mockPaymentRepo.findByPayPalOrderId.mockResolvedValue(mockPayment as any);
